...
df = timings.time_mist(ms=range(100,320,20), ns=[50], bs=[6], cs=[False], q=200)
...
df = timings.time_mist(ms=range(100,1100,100), ns=[50], bs=[4], cs=[False], q=50)
vec = df[df['alg'] == 'vector']
bit = df[df['alg'] == 'bitset']
m = vec['m']
v = vec['time'] * 1e6
...
ax.plot(m,b, 'r.', m, fit_bit(m), '--r',label='Bitset')
props = dict(boxstyle='round', facecolor='wheat', alpha=0.5)
ax.text(0.80, 0.10, "n=50,b=4", transform=ax.transAxes, fontsize=12,
                verticalalignment='top', bbox=props)
ax.legend()
ax.set_ylabel('us')
ax.set_xlabel('m')
plt.savefig("algorithm_compare3.tif")
