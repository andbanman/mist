...
DataMatrix::DataMatrix(int n, int m, int b) : n(n), m(m) {
    for (int ii = 0; ii < n; ii++) {
        vectors.push_back(mist::Variable::data_ptr(new mist::Variable::data_type[m]));
        for (int jj = 0; jj < m; jj++)
            vectors[ii].get()[jj] = 0;
        if (!b)
            throw DataMatrixException("DataMatrix", "Column " + std::to_string(ii) + " has zero value bins. Remove column from input data before proceeding.");
        bins.push_back(b);
    }
}

DataMatrix::DataMatrix(int data[], int n, int m) : n(n), m(m) {
    for (int ii = 0; ii < n; ii++) {
        vectors.push_back(mist::Variable::data_ptr(new mist::Variable::data_type[m]));
        int bin = 0;
        for (int jj = 0; jj < m; jj++) {
...
DataMatrix::DataMatrix(np::ndarray const& np) {
    ...
    if (np.get_dtype() != required_dtype)
        throw DataMatrixException("DataMatrix", "Invalid ndarray dtype, must be " + std::string(p::extract<char const*>(p::str(required_dtype))) + ", check numpy.ndarray.dtype");

    // Verify memory layout
    auto flags = np.get_flags();
    if (!(flags & np::ndarray::bitflag::C_CONTIGUOUS))
        throw DataMatrixException("DataMatrix", "Input ndarray is not C contiguous, check numpy.ndarray.flags");
...
DataMatrix::DataMatrix(std::string const& filename) {
    ...
    if (!ifs.is_open()) {
        throw DataMatrixException("DataMatrix", "Could not open input file '" + filename + "': " + std::strerror(errno));
    }
    ...
    // initialize vectors
    for (int ii = 0; ii < n; ii++)
        vectors.push_back(mist::Variable::data_ptr(new mist::Variable::data_type[m])); // TODO: valgrind error on new
    ...
    while(std::getline(ifs, line)) {
        ...
        for (auto c : line) {
            if (!std::isdigit(c) && pos) {
                ...
                jj++;
                if (jj >= n)
                    throw DataMatrixException("DataMatrix", "Error loading file " + filename + ":" + std::to_string(ii) + " - number of columns greater than expected.");
...
mist::Variable DataMatrix::getColumn(int i) {
    return mist::Variable(vectors[i], m, i, bins[i]);
};

Variable::tuple DataMatrix::variables() {
...
void DataMatrix::write_file(std::string const& filename, char sep) {
    std::ofstream ofs(filename);
    if (!ofs.is_open())
        throw DataMatrixException("write_file", "Could not open file " + filename + " for writing");
...
void DataMatrix::write_file(std::string const& filename) {
    write_file(filename, ',');
}

#ifdef BOOST_PYTHON_EXTENSIONS
void DataMatrix::python_write_file(std::string const& filename) { this->write_file(filename, ','); }
#endif
