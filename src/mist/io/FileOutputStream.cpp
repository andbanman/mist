...
FileOutputStream::init()
{
  if (!file || !file->is_open()) {
    throw FileOutputStreamException(
      "init",
      "Could not open file '" + filename +
        "' for wrtiting: " + std::strerror(errno));
  }
  if (!buffer) {
    throw FileOutputStreamException(
      "init",
      std::string("Could not allocate write buffer: ") + std::strerror(errno));
  }
  if (!double_strbuf.size()) {
    throw FileOutputStreamException(
      "init",
      std::string("Could not allocate double-to-string buffer: ") +
        std::strerror(errno));
  }
}
...
void
FileOutputStream::buffered_write(std::string const& ss)
{
  auto len = ss.length();
...
void
FileOutputStream::direct_write(std::string const& ss)
{
  std::unique_lock<mutex_type> lock(*this->m.get());
...
FileOutputStream::~FileOutputStream()
{
  if (buffer_cur_size) {
    try {
      ...
      std::cerr << "FileOutputStream::~FileOutputStream: Error: Could not "
                   "acquire lock, file '" +
  ...
  if (file.use_count() == 1) {
    try {
      ...
      std::cerr << "FileOutputStream::~FileOutputStream: Error closing file '" +
                     filename + "'.\n";
    }
  }
}
