...
//
Variable::Variable(data_ptr src, std::size_t size, std::size_t index, std::size_t bins)
    : data(src), _size(size), _index(index), _bins(bins) {
    if (!src.get())
        throw VariableException("Variable", "src stored pointer cannot be null", index);
    if (!size)
...
std::size_t Variable::bins() const  { return _bins; }
std::size_t Variable::size() const  { return _size; }
std::size_t Variable::index() const { return _index; }
...
//!
inline bool Variable::missing(std::size_t pos) const {
    if (pos >= _size)
        throw VariableOutOfRange("missing", _index, pos, _size);
    return this->data.get()[pos] < 0;
};

bool Variable::missingVal(Variable::data_type const val) {
...
Variable::data_type & Variable::at(std::size_t const pos) {
    if (pos >= _size)
        throw VariableOutOfRange("at", _index, pos, _size);
    return this->data.get()[pos];
};
Variable::data_type const& Variable::at(std::size_t const pos) const {
    if (pos >= _size)
        throw VariableOutOfRange("at", _index, pos, _size);
    return this->data.get()[pos];
};
...
//! of std::move.
bool Variable::operator==(Variable const& other) const noexcept {
...
Variable::const_iterator Variable::begin() const { return this->data.get(); };
Variable::const_iterator Variable::end() const { return this->data.get() + _size; };
Variable::iterator Variable::begin() { return this->data.get(); };
Variable::iterator Variable::end() { return this->data.get() + _size; };
