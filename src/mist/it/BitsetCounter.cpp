...
//
static void populateBitsetVariable(Variable const& v, BitsetVariable &b) {
        int bins = v.bins();
        int size = v.size();
...
static int bitsetCount(BitsetTable const& bitsetTable, Variable::tuple const& vars, Variable::indexes const& indexes, int vals[]) {
    int size = vars.size();
...
static void recursiveBitsetCount(BitsetTable const& bitsetTable, Variable::tuple const& vars,
        Variable::indexes const& indexes,
        Distribution &dist, int vals[], int pos) {
    int size = vars.size();
...
void static count1d(BitsetTable const& bitsetTable, Variable::tuple const& vars, Variable::indexes const& indexes, Distribution & dist) {
    int bins[1];
...
void static count2d(BitsetTable const& bitsetTable, Variable::tuple const& vars, Variable::indexes const& indexes, Distribution & dist) {
    int bins[2];
    auto n0 = vars[indexes[0]].bins();
    auto v0 = vars[indexes[0]].index();
...
void static count3d(BitsetTable const& bitsetTable, Variable::tuple const& vars, Variable::indexes const& indexes, Distribution & dist) {
    int bins[3];
    auto n0 = vars[indexes[0]].bins();
    auto v0 = vars[indexes[0]].index();
    auto n1 = vars[indexes[1]].bins();
...
Distribution BitsetCounter::count(Variable::tuple const& vars, Variable::indexes const& indexes) {
    ...
    switch(nvars) {
        ...
        case 1: count1d(this->bits, vars, indexes, dist); break;
        case 2: count2d(this->bits, vars, indexes, dist); break;
        case 3: count3d(this->bits, vars, indexes, dist); break;
        default:
            int vals[nvars];
...
//! @exception out_of_range Variable index out of range of table whose size set in constructor
Distribution BitsetCounter::count(Variable::tuple const& vars) {
...
Distribution BitsetCounter::count(Variable const& var) {
