...
public:
    SmallFilesOutOfRange(std::string const& method, std::string const& file, std::string const& key) :
        out_of_range("SmallFiles::" + method + " : File '" + file + "' for key " + key + "does not exist or could not be opened." ) { }
};
...
public:
    SmallFiles() : dir("./") {};
    SmallFiles(std::string const& dir) : dir(dir) {};
    ~SmallFiles() {};

    bool has(K const& key) {
        std::string filename = this->dir + "/" + std::to_string(boost::hash<K>{}(key));
